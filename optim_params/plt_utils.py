...
def plot_muscle_force(model, muscle_activations, q, q_dot, muscle_names=None):
    ...
    for i in range(q.shape[1]):
        ...
        for m in range(model.nbMuscles()):
            ...
            mus_fce[m, i] = (
                muscle_activations[m, i] * mus_tmp.FlCE(muscle_states[m]) * mus_tmp.FvCE()
            ) * mus_tmp.characteristics().forceIsoMax()
...
def plot_joint_torques(
    muscle_torques, reference_torques, residuals_torques=None, muscle_torque_optim=None, joint_names=None
):
