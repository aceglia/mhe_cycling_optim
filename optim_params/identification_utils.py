...
def _get_characteristics(muscle, name, use_mx=True):
    if name == "f_iso":
        ...
        return (
            muscle.characteristics().tendonSlackLength().to_mx()
            if use_mx
            else muscle.characteristics().tendonSlackLength()
        )
...
def get_initial_parameters(model, params_to_optim, use_mx=True):
    ...
    for p in params_to_optim:
        ...
        for m in range(model.nbMuscles()):
            ...
            if count == 0:
                ratio.append(
                    _get_characteristics(model.muscle(m), "lt_slack", use_mx=use_mx)
                    / _get_characteristics(model.muscle(m), "lm_optim", use_mx=use_mx)
                )
        count += 1
...
def compute_muscle_joint_torque(
    model,
    activations,
    q,
    qdot,
...
def return_muscle_torque_function(
    model, symbolics, p_mapping, with_param=True, params_to_optim=None, model_params_init=None, ratio=None
):
    mjt = compute_muscle_joint_torque(
        model,
        symbolics.x,
        symbolics.q,
        symbolics.qdot,
...
def return_muscle_length_function(
    model,
    q,
    with_param=True,
    p_mapping=None,
...
def get_cost_to_map(
    ...
    if "lm_optim" in params_to_optim:
        ...
        norm_len = muscle_len / (
            symbolics.lm_init * (symbolics.lm_optim / scaling_factor[1][params_to_optim.index("lm_optim")])
        )
...
def return_obj_function(obj_funct, act, q, qdot, tau, emg, p=None, tau_res=None, tau_muscle=None, lm_init=None):
    ...
    if tau_res is not None:
        if tau_muscle is not None:
            obj_tmp = obj_funct(act, q, qdot, tau, emg, tau_res, p, tau_muscle, lm_init)
        else:
...
def get_cost_n_dependant(
    ...
    for p_idx in range(len(p_mapping)):
        ...
        J_params += ca.sum1(weights[f"min_{params_to_optim[p_idx]}"] * ((p_tmp - scaling_factor[1][p_idx]) ** 2))
    ...
    if with_param:
        ...
        for p_idx, param in enumerate(params_to_optim):
            ...
            init_p = (
                ...
                if init_p is None
                else ca.vertcat(init_p, np.array(p_init[p_idx]) * scaling_factor[1][p_idx])
            )
            lb_p = (
...
def process_cycles(
    all_results, peaks, interpolation_size=120, remove_outliers=False, key__to_check=["q", "tau", "emg"]
):
