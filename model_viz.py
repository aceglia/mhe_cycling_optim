...
if __name__ == '__main__':
    ...
    files, part = get_all_file(["P10"], data_dir, to_include=["reference_torque", "gear_20"])
    data= load(files[0])
    q = data["q_ocp"]
    model = model_dir + f"/P10/models/gear_20_model_scaled_dlc_ribs_new_seth_param.bioMod"
    b = bioviz.Viz(model)
    b.load_movement(q)
    b.exec()