...
def plot_cycles(model, data_path, idx_to_export=0, cycles=True,color=None, line_style=None, optim_param_path=None, mhe_file=None, compare_to_fd=False):
    ...
    muscle_list = []
    for i in range(bio_model.nbMuscles()):
        muscle_list.append(bio_model.muscleNames()[i].to_string())
    ...
    init_joints_names = ["Pro/retraction", "Depression/Elevation",
                    ...
                    "Flexion/extension"]
    muscle_to_plot = ['Infraspinatus_I', 'Subscapularis_I', 'Supraspinatus_A', 'LevatorScapulae','Rhomboideus_I', 'TrapeziusClavicle_S',
                      'SerratusAnterior_S', 'PectoralisMajorThorax_M', 'PectoralisMinor',
                      ]
    names_to_plot = ['Infraspinatus', 'Subscapularis', 'Supraspinatus', 'Levator Scapula','Rhomboideus', 'Trapezius (Superior)',
    ...
    for key in key_to_plot:
        ...
        for count in range(len(muscle_to_plot)):
            i = muscle_list.index(muscle_to_plot[count])
            plt.subplot(ceil(len(muscle_to_plot) / 3), 3, count +1)
            ...
            for idx_j, j in enumerate(key_tmp):
                if j == "muscles_target" and i in track_idx:
                    ...
                    plt.margins(x=0)
                plt.title(names_to_plot[count] , fontsize=20)
