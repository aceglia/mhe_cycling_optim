...
def plot_param(data, muscle_to_plot, participants, muscle_names, plot_name="plot", color_palette=0, save_fig=True):
    ...
    def _plot_ridgeline(data, plot_name, color_palette):
        ...
        if color_palette == 1:
            pal = sn.cubehelix_palette(10, rot=0.25, light=0.7)
        else:
            pal = sn.cubehelix_palette(10, rot=-0.25, light=0.7)
    ...
    for p in range(len(participants)):
        ...
        pd_tmp["muscles"] = sum(
            [
                [muscle_names[i]] * n_batch * len(nodes)
                for i in range(len(muscle_names))
                if muscle_names[i] in muscle_to_plot
            ],
            [],
...
if __name__ == "__main__":
    ...
    pd_p_iso["cycles"] = sum(
        [[f"cycle_{i}"] * n_batch * len(muscle_names) * len(participants) for i in range(len(nodes))], []
    )
    pd_l_iso = pd.DataFrame()
    pd_l_iso["muscles"] = sum(
        [[muscle_names[i]] * n_batch * len(participants) for i in range(len(muscle_names))], []
    ) * len(nodes)
    pd_l_iso["cycles"] = sum(
        [[f"cycle_{i}"] * n_batch * len(muscle_names) * len(participants) for i in range(len(nodes))], []
