...
if __name__ == "__main__":
    ...
    for p, part in enumerate(participants):
        ...
        for t, node in enumerate(nodes):
            ...
            p_iso_all_node[count : count + len(muscle_names) * n_batch_tmp] = np.array(final_list_p).reshape(
                len(muscle_names) * n_batch_tmp
            )
    ...
    for i in range(len(nodes)):
        ...
        for k in range(2):
            ...
            value_per_tab[k, 0, i] = (
                np.mean(value_param_p[:, i][value_param_p[:, i] != 0])
                + np.mean(value_param_l[:, i][value_param_l[:, i] != 0])
            ) / 2
        ...
        print(
            f"{b_i}{i+1}{b_e} & {b_i}{value_per_tab[1, 0, i]:0,.2f}{b_e}  & {b_i}{value_per_tab[0, 1, i]:0,.2f}{b_e}   &"
