...
def compute_muscle_tau(model_path, a, q, qdot, param, param_list):
    ...
    for j in range(2):
        ...
        if j == 1:
            model_tmp = apply_params(
                model_tmp, params=param, optimized_params=param_list, with_casadi=False, ratio=ratio
            )
        ...
        for n, trial in enumerate(trials):
            for t, node in enumerate(nodes):
                for d, dy in enumerate(dyn):
                    ...
                    emg = data_tmp["emg"]
                    mus_tau = compute_muscle_tau(
                        model,
                        data_tmp["a"],
                        data_tmp["q"],
                        data_tmp["qdot"],
                        data_tmp["p"],
                        param_list=data_tmp["optimized_params"],
                    )
                    ...
                    print(
                        prefix
                        + before
                        + f'& {dy} & {all_f_iso[:, t, d].mean():0,.2f} & {all_l_optim[:, t, d].mean():0,.2f} & {emg_error:0,.2f} & {tau_error:0,.2f} & {color} {data_tmp["solver_out"]["n_iter"]} & {data_tmp["solving_time"]:0,.2f}'
                        + r"\\"
                    )
